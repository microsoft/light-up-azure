...
8. Review 화면이 나오면 **만들기** 버튼을 눌러 vNet을 생성합니다.
...
7. 아래와 같이 Tag를 구성 후 **검토+만들기** 버튼을 클릭하여 VM을 생성합니다.
...
9. 위와 동일한 방법으로 퍼블릭 서브넷에도 VM을 배포해 보도록 하겠습니다.
...
11. 아래와 같이 네트워킹을 구성하고, **태그**를 추가한 다음, **검토 + 만들기** 버튼을 클릭해 VM을 생성합니다.
...
2. 리소스에 액세스하는 다양한 방법이 있지만, 이번 실습에서는 공유 액세스 토큰을 사용해 보도록 하겠습니다.  생성한 컨테이너를 선택 후, 왼쪽 메뉴에서 **공유 액세스 토큰**을 클릭합니다.
3. 기본 설정을 그대로 두고 SAS **토큰 및 URL 생성** 버튼을 클릭합니다.
4. Blob SAS URL을 복사해 줍니다.

![Untitled](statics/Untitled%2044.png)

5. 퍼블릭 서브넷에 있는 VM을 배스천 호스트로 사용하여 프라이빗 서브넷에 있는 VM에 접속할 것입니다. SSH 연결 시 SSH 프라이빗 키가 필요하기 때문에 PublicVM에 SSH 프라이빗 키를 업로드 하도록 하겠습니다.

```powershell
scp -i <private key path> <key> azureuser@<vm-ip>:/home/azureuser
```

6. 이제 PublicVM에 접속해 줍니다.

```powershell
ssh -i <private key path> azureuser@<vm-ip>
```

7. 업로드한 SSH 프라이빗 키가 /home/azureuser 경로에 있는 것을 확인 후, 아래 명령어를 통해 읽기 전용 액세스 권한을 지정합니다.

```powershell
chmod 400 <keyname>.pem
```

8. 아래의 명령을 실행하여 PrivateVM에 연결합니다.

```powershell
ssh -i <private key path> azureuser@<vm-ip>
```

![Untitled](statics/Untitled%2045.png)

먼저 `wget` 명령어를 통해 PrivateVM에서 서비스 엔드포인트를 통해 리소스를 잘 가져오는지 테스트합니다.

![Untitled](statics/Untitled%2046.png)
...
![Untitled](statics/Untitled%2047.png)
